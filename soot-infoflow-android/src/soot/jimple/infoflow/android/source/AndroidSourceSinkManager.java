...
public class AndroidSourceSinkManager implements ISourceSinkManager, IOneSourceAtATimeManager {
     ...
     * @param sources         The list of source methods
     * @param sinks           The list of sink methods
     * @param callbackMethods The list of callback methods whose parameters are sources through
     *                        which the application receives data from the operating system
     * @param weakMatching    True for weak matching: If an entry in the list has no return
     ...
     * @param config          The configuration of the data flow analyzer
     * @param layoutControls  A map from reference identifiers to the respective Android layout
     *                        controls
     */
    public AndroidSourceSinkManager(Set<SourceSinkDefinition> sources, Set<SourceSinkDefinition> sinks,
     ...
     * @param manager   The manager object providing access to the configuration and the
    ...
    protected SourceSinkDefinition getSourceDefinition(SootMethod method) {
        if (oneSourceAtATime) {
            if (osaatType == SourceType.MethodCall && currentSource == method)
                return createSourceSinkDefinition(method);
            else
    ...
    private SourceSinkDefinition createSourceSinkDefinition(SootMethod method) {
     ...
     *
     * @param sCallSite The call site that may potentially read data from a sensitive UI
     *                  control
     ...
     * as sources
     *
     * @param resourcePackages The resource packages to be used for looking up layout controls
     */
